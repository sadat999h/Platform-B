...
app.get('/api/video/:videoId', async (req, res) => {
  try {
    ...
    console.log('Security string received:', securityString ? 'Yes' : 'No');
    console.log('Expected security string:', MASTER_SECURITY_STRING ? 'Set' : 'Not set');
...
app.get('/api/stream/:videoId', async (req, res) => {
  try {
    ...
    console.log('Query key:', req.query.key ? req.query.key.substring(0, 20) + '...' : 'none');
    console.log('Security present:', !!securityString);
    console.log('Master set:', !!MASTER_SECURITY_STRING);
    
    if (!securityString) {
      console.error('No security string provided');
      return res.status(403).send('Forbidden: No security string');
    }
    
    // Trim and compare
    const cleanSecurity = String(securityString).trim();
    const cleanMaster = String(MASTER_SECURITY_STRING).trim();
    
    console.log('Lengths - Received:', cleanSecurity.length, 'Master:', cleanMaster.length);
    
    if (cleanSecurity !== cleanMaster) {
    ...
    console.log('Source URL set:', !!sourceUrl);
    ...
    try {
      ...
      // Get content type from source or default to video/mp4
      let contentType = response.headers.get('content-type');
      
      // If no content-type or wrong type, set based on URL
